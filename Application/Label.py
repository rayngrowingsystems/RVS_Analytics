...
class Label(QLabel):
    ...
    def paintEvent(self, e):  # Qt override, keep casing
        ...
        if self.magnifier_mode and not self.pixmap().isNull():
            ...
            # Scale the pixmap to the same size as the original image.
            # scale_factor is delivered through the magnifier_zoom_factor
            full_original_size = QSize(self.pixmap().width() / self.magnifier_zoom_factor,
                                       self.pixmap().height() / self.magnifier_zoom_factor)
            magnified_full_pixmap = self.pixmap().scaled(full_original_size, QtCore.Qt.KeepAspectRatio,
            ...
            source_rect = QRect(scaled_center_point.x() - self.magnifier_size / 2,
                                scaled_center_point.y() - self.magnifier_size / 2,
                                self.magnifier_size, self.magnifier_size) # QRect(top_left_point, magnifier_size)

            magnified_zoom_pixmap = magnified_full_pixmap.copy(source_rect)

            # Paint it
            painter_half_magnifier_size = self.magnifier_size / 2
            painter_top_left_point = self.magnifier_pos - QPoint(painter_half_magnifier_size,
                                                                 painter_half_magnifier_size)
